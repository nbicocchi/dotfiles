...
function ll --description "Scroll ll if theres more files that fit on screen"
  eza --long $argv --color=always | less -R -X -F
end
...
function update-mirrors --description "Update Arch Mirrors"
  set TMPFILE (mktemp)
  set MIRROR_LIST "/etc/pacman.d/mirrorlist"
  set MIRROR_BACK "/etc/pacman.d/mirrorlist.backup"

  sudo true
  echo "--> Fetching and ranking mirror list..."
  curl -s "https://archlinux.org/mirrorlist/?country=FR&country=IT&country=DE&country=CH&protocol=https&use_mirror_status=on" | \
    sed -e "s/^#Server/Server/" -e "/^#/d" | rankmirrors -v -n 10 - | tee $TMPFILE || begin
      echo "Failed"
      return
    end

  echo "--> Updating $MIRROR_LIST..."
  if test -r $MIRROR_LIST
    sudo mv $MIRROR_LIST $MIRROR_BACK
  end
